...
local hyperKey = function(ev)
	-- local pressed = true
	local cmd  = ev:getFlags().cmd
	local alt  = ev:getFlags().alt
	local ctrl = ev:getFlags().ctrl
	local shift = ev:getFlags().shift
	return cmd and alt and ctrl and shift
end
...
module.eventWatcher = eventtap.new({events.flagsChanged, events.keyDown}, function(ev)
    -- if it's been too long; previous state doesn't matter
		...
		if ev:getType() == events.flagsChanged then
            ...
            timeFirstControl, firstDown, secondDown = 0, false, false
				elseif hyperKey(ev) and not firstDown then         -- ctrl down and it's a first
            firstDown = true
            timeFirstControl = timer.secondsSinceEpoch()
				elseif hyperKey(ev) and firstDown then             -- ctrl down and it's the second
            secondDown = true
